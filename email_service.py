...
def send_attendance_email(parent_email: str, student_name: str,
                          timestamp: datetime = None, status: str = "present"):
    ...
    if is_absent:
        ...
        badge_label  = "Absent"
        badge_icon   = "⚠️"
        subject      = f"⚠️ {student_name} was absent today — {date_str}"
        header_color = "#7f1d1d"
        header_title = "⚠️ Absence Alert"
        body_text    = (
            f'Your child <strong>{student_name}</strong> was '
            f'<strong style="color:#ef4444;">absent</strong> today and '
            f'<strong>did not mark attendance</strong>.'
        )
    elif is_late:
        badge_color  = "#f59e0b"
        badge_label  = "Late"
        badge_icon   = "🕐"
        subject      = f"🕐 {student_name} arrived late — {date_str}"
        header_color = "#78350f"
        header_title = "🕐 Late Attendance"
        body_text    = (
            f'Your child <strong>{student_name}</strong> was marked '
            f'<strong style="color:#f59e0b;">late</strong> today.'
        )
    else:
        badge_color  = "#22c55e"
        badge_label  = "Present"
        ...
        body_text    = (
            f'Your child <strong>{student_name}</strong> has been marked '
            f'<strong style="color:#22c55e;">present</strong> today.'
        )

    time_row = "" if is_absent else f"""
        <tr>
    ...
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;
      ...
      <div style="padding:28px 32px;background:#fff;">
        ...
        <p style="margin:0 0 24px;color:#333;font-size:15px;">{body_text}</p>
        <div style="background:#f4f7ff;border-radius:6px;padding:16px 20px;margin-bottom:24px;">
          <table style="width:100%;border-collapse:collapse;">
            ...
            <tr>
              ...
              <td style="color:{badge_color};font-size:13px;font-weight:bold;text-align:right;">
                {badge_icon} {badge_label}
              </td>
            </tr>
          </table>
        </div>
        <p style="margin:0;color:#888;font-size:12px;">This is an automated message. Please do not reply.</p>
      </div>
    ...
    try:
        ...
        print(f"Email ({status}) sent to {parent_email}")
        return True
